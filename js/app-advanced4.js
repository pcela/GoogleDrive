  ...
  <div class="modal fade" id="Model" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="SearchModelLabel" aria-hidden="true">
  ...
  // <li class="breadcrumb-item"><a href="${item}">${navnamecr}</a></li>
    ...
    content += `</ol>
      ...
      <div id="list" class="list-group text-break">
    	...
    	<div class="${UI.file_count_alert_class} text-center d-none" role="alert" id="count">Razem <span class="number text-center"></span> items</div>
   ...
   */
   function onSearchResultItemClick(a_ele) {
       ...
       $.post(`${searchhost}:id2path`, {
           ...
           title = `Failed`;
           $('#SearchModelLabel').html(title);
           content = `Nie można pobrać łącza do pliku/folderu. Spróbuj ponownie.`;
           $('#modal-body-space').html(content);
       })
   }
  ...
  function file_video(path) {
    ...
    <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${obj.name}<br>${size}</div>
  	<video id="vplayer" width="100%" height="100%" playsinline controls: ['play-large', 'play', 'progress', 'current-time', 'mute', 'volume', 'captions', 'settings', 'pip', 'airplay', 'fullscreen']; data-plyr-config="{ "title": "${decodename}"}" data-poster="${poster}" style="--plyr-captions-text-color: #ffffff;--plyr-captions-background: #000000;">
  	  ...
  	  <track kind="captions" label="Default" src="${caption}.vtt" srclang="pl" />
