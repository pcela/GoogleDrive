...
<input class="form-control me-2" name="q" type="search" placeholder="Search" aria-label="Szukaj" value="${search_text}" required>
...
function onSearchResultItemClick(a_ele) {
    ...
    $.post(`/${cur}:id2path`, {
        ...
        if (data) {
            ...
            title = `Result`;
            $('#SearchModelLabel').html(title);
            content = `<a class="btn btn-info" href="${ehrefurl}">Open</a> <a class="btn btn-secondary" href="${ehrefurl}" target="_blank">Otwórz w nowej karcie</a>`;
            $('#modal-body-space').html(content);
            return;
        }
        title = `Failed`;
...
<div class="card text-center">
    <div class="card-body text-center">
      <div class="${UI.file_view_alert_class}" id="file_details" role="alert"><b>404.</b> Wystąpił błąd</div>
    </div><p>Żądany adres URL nie został znaleziony na tym serwerze. To wszystko co wiemy.</p>
      <div class="card-text text-center">
      <div class="btn-group text-center">
        <a href="/" type="button" class="btn btn-primary">Strona główna</a>
      </div>
        </div><br>
</div>
</div>`
