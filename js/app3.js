...
<input class="form-control me-2" name="q" type="search" placeholder="Search" aria-label="Wpisz co szukasz" value="${search_text}" required>
...
// <li class="breadcrumb-item"><a href="${item}">${navnamecr}</a></li>
  ...
  content += `</ol>
    ...
    <div id="list" class="list-group text-break">
  	...
  	<div class="${UI.file_count_alert_class} text-center d-none" role="alert" id="count">Łącznie <span class="number text-center"></span> items</div>
    ...
    requestListPath(path, {
        ...
        function(path) {
            $('#spinner').remove();
            var pass = prompt("Katalog zaszyfrowany, podaj hasło", "");
...
function render_search_result_list() {
  ...
  <div class="card">
  <div class="${UI.path_nav_alert_class} d-flex align-items-center" role="alert" style="margin-bottom: 0;">Wyniki wyszukowania</div>
  <div id="list" class="list-group text-break">
  </div>
  </div>
  <div class="${UI.file_count_alert_class} text-center d-none" role="alert" id="count">Łącznie <span class="number text-center"></span> items</div>
...
function onSearchResultItemClick(a_ele) {
    ...
    $.post(`/${cur}:id2path`, {
        ...
        if (data) {
            ...
            title = `Result`;
            $('#SearchModelLabel').html(title);
            content = `<a class="btn btn-info" href="${ehrefurl}">Otwórz</a> <a class="btn btn-secondary" href="${ehrefurl}" target="_blank">Otwórz w nowej karcie</a>`;
            $('#modal-body-space').html(content);
            return;
        }
        title = `Failed`;
        $('#SearchModelLabel').html(title);
        content = `Nie można pobrać łącza do pliku/folderu. Odśwież stronę i spróbuj ponownie.`;
        $('#modal-body-space').html(content);
    })
}
...
<div class="input-group mb-4">
  ...
  <div class="btn-group text-center">
      <a href="${url}" type="button" class="btn btn-primary">Pobierz</a>
...
function file_video(path) {
  ...
  <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${obj.name}<br>${size}</div>
    ...
    <track kind="captions" label="Polish" src="${caption}.en.vtt" srclang="pl" default />
    <track kind="captions" label="English" src="${caption}.en.vtt" srclang="en" />
